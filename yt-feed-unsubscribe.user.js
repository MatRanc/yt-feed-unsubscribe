...
(() => {
  ...
  const CARD = 'ytd-rich-item-renderer, yt-lockup-view-model, ytd-video-renderer, ytd-grid-video-renderer';
  ...
  function addItem(menu, href, name) {
    ...
    item.querySelector('.ytListItemViewModelTitle').textContent = `Unsubscribe from ${name}`;
    ...
    item.addEventListener('click', async e => {
      ...
      try {
        await api('subscription/unsubscribe', { channelIds: [await channelId(href)] });
        // Hide the channel's other videos in the feed too.
        document.querySelectorAll(`a[href="${CSS.escape(href)}"]`).forEach(a => {
          const card = a.closest('ytd-rich-item-renderer') || a.closest(CARD);
  ...
  document.addEventListener('click', e => {
    ...
    const link = card?.querySelector(CHANNEL_LINK); // Shorts cards have no channel link; skip them
    if (!link) return;
    const href = link.getAttribute('href');
    const name = link.textContent.trim() || href.slice(1);
    ...
    const timer = setInterval(() => {
      ...
      if (menu) addItem(menu, href, name);
    }, 50);
  }, true);
})();
